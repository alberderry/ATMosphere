...
import bjbBackground from '../assets/img/bg-bjb.jpg'; // Path gambar background
...
const Login = () => {
  ...
  // Menghapus 'isAuthenticated' karena tidak digunakan di komponen ini
  const { login, isLoading: authLoading, error: authError } = useAuth(); // Ambil authLoading dan authError
  const toast = useToast();
  const navigate = useNavigate();

  // State lokal untuk animasi showLoginForm
  const [showLoginForm, setShowLoginForm] = useState(false);

  // Fungsi handleLogin yang akan memanggil fungsi login dari AuthContext
  const handleLogin = async () => {
    const success = await login(email, password); // Langsung panggil fungsi login dari AuthContext dengan email & password
